...
1. **Finish the session-format rollout** — `refresh-dev-standards.sh` has been run in all 13 downstream repos (2026-09-25); the results are uncommitted there. Merge `feature/rollout-fixes`, re-run refresh in `lifeos-core`, `lifeos-envprofiler`, `lifeos-ingestor`, `lifeos-mcp` and `lifeos-neo4j` to repoint their dangling `GEMINI.md -> claude.md` symlinks, then commit + push each repo (`lifeos-collector` is on a `claude/…` branch, not `develop`). Then fix the `lifeos-mcp` strategic-review query (see Technical Debt)
